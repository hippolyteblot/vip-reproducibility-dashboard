                                    ...
                                    html.Div(
                                        children=[
                                            ...
                                            html.Div(
                                                children=[
                                                    html.P(
                                                        'Select an experiment to study its results. Experiment results are stored on '
                                                        'Girder (the VIP platform data storage) with associated metrics values. '
                                    ...
                                    html.Div(
                                        children=[
                                            ...
                                            html.Div(
                                                children=[
                                                    html.P(
                                                        'Select two experiments to compare their results, and study similarity metrics. '
                                                    ),
                                                    dbc.Button(
                                                        "Compare experiments",
                                                        id="exp-open",
...
def filter_exp(version_id):
    ...
    options = [{'label': exp['name'], 'value': exp['id']} for exp in new_exp_list]
...
def toggle_modal_exp(n1, n2, is_open):
    if n1 or n2:
        exp_list = load_exp_from_db()
        exp_options = [{'label': exp['name'], 'value': str(exp['id']) + '/-/' + exp['application_name']} for exp in exp_list]
        applications = get_available_applications()
        options = [{'label': app['name'], 'value': str(app['id']) + '/-/' + app['name']} for app in applications]
        options.insert(0, {'label': 'All', 'value': 'all'})
        return not is_open, exp_options, exp_options, options
